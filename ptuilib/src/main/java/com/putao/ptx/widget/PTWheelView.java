...
/**
 * @ClassName : {@link PTWheelView}
 * @Description : 滚轮选择器
 *
 * @Author xiaosw<xiaoshiwang@putao.com>
 * @Date 2017-02-24 10:10:20
 */
public class PTWheelView extends View {

    /** @see PTWheelView#getClass().getSimpleName() */
    private static final String TAG = "PTWheelView";
    /**
    ...
    // Scrolling
    private PTWheelViewAdapter.WheelScroller mWheelScroller;
    ...
    // View adapter
    private PTWheelViewAdapter mPTWheelViewAdapter;

    // Recycle
    private WheelRecycle mRecycle = new WheelRecycle(this);

    // Listeners
    private List<PTOnWheelViewChangedListener> mChangingListeners =
            new LinkedList<PTOnWheelViewChangedListener>();
    private List<PTOnWheelViewScrollListener> mScrollingListeners =
            new LinkedList<PTOnWheelViewScrollListener>();
    private List<PTOnWheelViewClickedListener> mWheelViewClickingListeners =
            new LinkedList<PTOnWheelViewClickedListener>();

    /**
     * Constructor
     */
    public PTWheelView(Context context) {
        super(context);
        initData();
    }
     ...
     * Constructor
     */
    public PTWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initData();
    }

    public PTWheelView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initData() {
        mWheelScroller = new PTWheelViewAdapter.WheelScroller(getContext(), mScrollingListener);
    }

    PTWheelViewAdapter.WheelScroller.ScrollingListener mScrollingListener =
            new PTWheelViewAdapter.WheelScroller.ScrollingListener() {
        ...
        public void onJustify() {
            if (Math.abs(mScrollingOffset) > PTWheelViewAdapter.WheelScroller.MIN_DELTA_FOR_SCROLLING) {
     ...
     * depends on mPTWheelView layout parameters. To apply changes and rebuild view
     * call measure().
     *
     ...
     */
    public PTWheelViewAdapter getPTWheelViewAdapter() {
        return mPTWheelViewAdapter;
    }
     ...
     *
     * @param PTWheelViewAdapter the view adapter
     */
    public void setPTWheelViewAdapter(PTWheelViewAdapter PTWheelViewAdapter) {
        if (this.mPTWheelViewAdapter != null) {
            this.mPTWheelViewAdapter.unregisterDataSetObserver(mDataObserver);
        }
        this.mPTWheelViewAdapter = PTWheelViewAdapter;
        if (this.mPTWheelViewAdapter != null) {
            this.mPTWheelViewAdapter.registerDataSetObserver(mDataObserver);
        }
    ...
    /**
     * Adds mPTWheelView changing listener
     *
     * @param listener the listener
     */
    public void addWheelViewChangingListener(PTOnWheelViewChangedListener listener) {
        mChangingListeners.add(listener);
    }

    /**
     * Removes mPTWheelView changing listener
     *
     * @param listener the listener
     */
    public void removeWheelViewChangingListener(PTOnWheelViewChangedListener listener) {
     ...
     *
     * @param oldValue the old mPTWheelView value
     * @param newValue the new mPTWheelView value
     */
    protected void notifyChangingListeners(int oldValue, int newValue) {
        for (PTOnWheelViewChangedListener listener : mChangingListeners) {
            listener.onChanged(this, oldValue, newValue);
        }
    }

    /**
     * Adds mPTWheelView scrolling listener
     *
     * @param listener the listener
     */
    public void addWheelViewScrollingListener(PTOnWheelViewScrollListener listener) {
        mScrollingListeners.add(listener);
    }

    /**
     * Removes mPTWheelView scrolling listener
     *
     * @param listener the listener
     */
    public void removeWheelViewScrollingListener(PTOnWheelViewScrollListener listener) {
        mScrollingListeners.remove(listener);
    }
    /**
     * Removes all mPTWheelView scrolling listener
     */
    ...
    protected void notifyScrollingListenersAboutStart() {
        for (PTOnWheelViewScrollListener listener : mScrollingListeners) {
    ...
    /**
     * Adds mPTWheelView clicking listener
     *
     * @param listener the listener
     */
    public void addWheelViewClickingListener(PTOnWheelViewClickedListener listener) {
        mWheelViewClickingListeners.add(listener);
    }

    /**
     * Removes mPTWheelView clicking listener
     *
     * @param listener the listener
     */
    public void removeWheelViewClickingListener(PTOnWheelViewClickedListener listener) {
        mWheelViewClickingListeners.remove(listener);
    }

    /**
     * Removes all mPTWheelView clicking listener
     */
    ...
    protected void notifyClickListenersAboutClick(int item) {
        for (PTOnWheelViewClickedListener listener : mWheelViewClickingListeners) {
    ...
    public void setCurrentItem(int index, boolean animated) {
        if (mPTWheelViewAdapter == null || mPTWheelViewAdapter.getItemsCount() == 0) {
            return; // throw?
        }

        int itemCount = mPTWheelViewAdapter.getItemsCount();
    ...
    /**
     * Tests if mPTWheelView is cyclic. That means before the 1st item there is shown
     * the last one
     *
     * @return true if mPTWheelView is cyclic
     */
    ...
    /**
     * Returns height of mPTWheelView item
     *
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!isEnabled() || getPTWheelViewAdapter() == null) {
            return true;
        }
    ...
    /**
     * Calculates range for mPTWheelView mViews
     *
    ...
    /**
     * Rebuilds mPTWheelView mViews if necessary. Caches all unused mViews.
     *
     ...
     *
     * @param index the item index
     * @return true if item index is not out of bounds or the mPTWheelView is cyclic
     */
    private boolean isValidItemIndex(int index) {
        return mPTWheelViewAdapter != null && mPTWheelViewAdapter.getItemsCount() > 0 &&
                (isCyclic || index >= 0 && index < mPTWheelViewAdapter.getItemsCount());
    }
    ...
    private View getItemView(int index) {
        ...
        int count = mPTWheelViewAdapter.getItemsCount();
        if (!isValidItemIndex(index)) {
            return mPTWheelViewAdapter.getEmptyItem(mRecycle.getEmptyItem(), mLinearLayout);
        } else {
            while (index < 0) {
                index = count + index;
            }
        ...
        index %= count;
        return mPTWheelViewAdapter.getItem(index, mRecycle.getItem(), mLinearLayout);
    }
    ...
    protected class WheelRecycle {
         ...
         * Constructor
         *
         * @param wheel the mPTWheelView view
         */
        public WheelRecycle(PTWheelView wheel) {
            this.mPTWheelView = wheel;
        }
         ...
         * @param range     the range of current mPTWheelView mViews
        ...
        private void recycleView(View view, int index) {
            int count = mPTWheelView.getPTWheelViewAdapter().getItemsCount();

            if ((index < 0 || index >= count) && !mPTWheelView.isCyclic()) {
                // empty view
     ...
     * <p/>
     * The onItemClicked() method is called whenever a mPTWheelView item is clicked
     * <li>New Wheel position is set
     * <li>Wheel view is scrolled
     */
    public interface PTOnWheelViewClickedListener {
        /**
         ...
         */
        void onItemClicked(PTWheelView wheel, int itemIndex);

    }

    public interface PTOnWheelViewChangedListener {
        /**
         * Callback method to be invoked when current item changed
         *
         * @param wheel    the mPTWheelView view whose state has changed
         * @param oldValue the old value of current item
         * @param newValue the new value of current item
         */
        void onChanged(PTWheelView wheel, int oldValue, int newValue);
    }

    public interface PTOnWheelViewScrollListener {
        /**
         * Callback method to be invoked when scrolling started.
         *
         * @param wheel the mPTWheelView view whose state has changed.
         */
        void onScrollingStarted(PTWheelView wheel);
         ...
         * @param wheel the mPTWheelView view whose state has changed.
         */
        void onScrollingFinished(PTWheelView wheel);
    }
