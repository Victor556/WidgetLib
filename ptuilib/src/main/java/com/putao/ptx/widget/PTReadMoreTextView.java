...
/**
 * @author WhiteTec 2017/7/17
 */
public class PTReadMoreTextView extends AppCompatTextView {
    ...
    //  used to judge if the listener of corresponding to the onclick event of ReadMoreTextView
    //  is specifically for inner toggle
    private ExpandableClickListener mExpandableClickListener;
    private OnExpandListener mOnExpandListener;

    public PTReadMoreTextView(Context context) {
        super(context);
        init();
    }

    public PTReadMoreTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init();
    }

    public PTReadMoreTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PTReadMoreTextView);
        if (a == null) {
            return;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.PTReadMoreTextView_rtv_MaxLinesOnShrink) {
                mMaxLinesOnShrink = a.getInteger(attr, MAX_LINES_ON_SHRINK);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_EllipsisHint) {
                mEllipsisHint = a.getString(attr);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToExpandHint) {
                mToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToShrinkHint) {
                mToShrinkHint = a.getString(attr);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_EnableToggle) {
                mToggleEnable = a.getBoolean(attr, TOGGLE_ENABLE);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToExpandHintShow) {
                mShowToExpandHint = a.getBoolean(attr, SHOW_TO_EXPAND_HINT);
            } else if (attr == R.styleable.PTReadMoreTextView_etv_ToShrinkHintShow) {
                mShowToShrinkHint = a.getBoolean(attr, SHOW_TO_SHRINK_HINT);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToExpandHintColor) {
                mToExpandHintColor = a.getInteger(attr, TO_EXPAND_HINT_COLOR);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToShrinkHintColor) {
                mToShrinkHintColor = a.getInteger(attr, TO_SHRINK_HINT_COLOR);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToExpandHintColorBgPressed) {
                mToExpandHintColorBgPressed = a.getInteger(attr, TO_EXPAND_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_ToShrinkHintColorBgPressed) {
                mToShrinkHintColorBgPressed = a.getInteger(attr, TO_SHRINK_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_InitState) {
                mCurrState = a.getInteger(attr, STATE_SHRINK);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_GapToExpandHint) {
                mGapToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.PTReadMoreTextView_rtv_GapToShrinkHint) {
    ...
    private void init() {
        ...
        if (TextUtils.isEmpty(mToExpandHint)) {
            mToExpandHint = getResources().getString(R.string.read_more);

        }
        if (TextUtils.isEmpty(mToShrinkHint)) {
            mToShrinkHint = getResources().getString(R.string.read_less);
        }
        if (mToggleEnable) {
    ...
    /**
     * used in ListView or RecyclerView to update ReadMoreTextView
     *
     * @param text                original text
     * @param futureTextViewWidth the width of ReadMoreTextView in px unit,
    ...
    /**
     * get the current state of ReadMoreTextView
     *
    ...
    private CharSequence getNewTextByConfig() {
        ...
        switch (mCurrState) {
            case STATE_SHRINK: {
                ...
                int indexEndTrimmed = indexEnd - getLengthOfString(mEllipsisHint) - (mShowToExpandHint ?
                        getLengthOfString(mToExpandHint) + getLengthOfString(mGapToExpandHint) : 0);
                ...
                int remainWidth = getValidLayout().getWidth() - (int) (mTextPaint.measureText(mOrigText.subSequence
                        (indexStart, indexEndTrimmed).toString()) + 0.5);
                float widthTailReplaced = mTextPaint.measureText(getContentOfString(mEllipsisHint) +
                        (mShowToExpandHint ? (getContentOfString(mToExpandHint) + getContentOfString
                                (mGapToExpandHint)) : ""));
                ...
                if (remainWidth > widthTailReplaced) {
                    ...
                    while (remainWidth > widthTailReplaced + extraWidth) {
                        ...
                        if (indexEndTrimmed + extraOffset <= mOrigText.length()) {
                            extraWidth = (int) (mTextPaint.measureText(mOrigText.subSequence(indexEndTrimmed,
                    ...
                    while (remainWidth + extraWidth < widthTailReplaced) {
                        ...
                        if (indexEndTrimmed + extraOffset > indexStart) {
                            extraWidth = (int) (mTextPaint.measureText(mOrigText.subSequence(indexEndTrimmed +
                                    extraOffset, indexEndTrimmed).toString()) + 0.5);
                        } else {
                            break;
                        }
                    }
            ...
            case STATE_EXPAND: {
                ...
                ssbExpand.setSpan(mTouchableSpan, ssbExpand.length() - getLengthOfString(mToShrinkHint), ssbExpand
    ...
    public interface OnExpandListener {
        void onExpand(PTReadMoreTextView view);

        void onShrink(PTReadMoreTextView view);
    }
    ...
    private class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if (hasOnClickListeners() && (getOnClickListener(PTReadMoreTextView.this) instanceof ExpandableClickListener)) {
            } else {
                toggle();
            }
        }
